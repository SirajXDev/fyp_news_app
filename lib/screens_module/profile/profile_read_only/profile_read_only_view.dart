...
class ProfileReadOnlyView extends StatelessWidget {
  const ProfileReadOnlyView({super.key, this.onTap});
      ...
      child: RefreshIndicator(
        onRefresh: () async {
          context.read<ProfileBloc>().add(
            ProfileGetEvent(id: FirebaseAuth.instance.currentUser!.uid),
          );
        },
        child: ListView(
          padding: EdgeInsets.symmetric(
            horizontal: context.mqh * .02,
  ...
  Widget _buildProfileDetails(BuildContext context, Profile? profile) {
    return Column(
      ...
      children: [
        ...
        ProfileReadOnlyFieldView(
          labelText: 'Bio',
          title: profile?.bio ?? 'hey! this profile is in the testing mode..',
        ),
      ],
    );
  }
